...
Phase 12 retains `report.view_sales` and `report.view_profit`, and adds `report.view_inventory`, `report.view_purchases`, `report.view_customers`, `report.view_suppliers`, `report.view_expenses`, and `report.view_cash`. Profit requires both sales and profit permissions. Dashboard domains are omitted when their granular permission is absent. CSV exports perform the permission check selected by export kind and reuse authenticated company/active-branch scope.

Phase 13 adds `audit.view`. Audit retrieval is company-scoped and independently permissioned; audit creation remains an internal consequence of the protected business command and does not require an additional caller permission.
